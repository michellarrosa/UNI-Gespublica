<?php
	...
	if(array_key_exists('atividade_dataini',$licform) && is_null($licform['atividade_dataini'])){
...
foreach ($quadrante_checkbox as $key => $value) {
	if(!isset($licform[$key])){
		$licform[$key]=$value;
	}
}
...
###################################################################### INSERÇÕES MANUAIS
	...
	if($_SESSION['uni_ext_form_op'] == "envio"){
		$query = "INSERT INTO uni__licam_empreendimentodata (" . implode(',', $campos) . ") VALUES (" . implode(',', $valores) . ")";
	}else{
		$query = "UPDATE uni__licam_empreendimentodata SET ".$update." WHERE id = ".$_SESSION['uni_ext_id_form'];		
	}
